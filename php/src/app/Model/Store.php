...
class Store extends Model
{
    ...
    // Override find method to normalize logo path
    public function find($id) {
        $store = parent::find($id);
        if ($store) {
            $store = $this->normalizeLogoPath($store);
        }
        return $store;
    }

    // Override first method to normalize logo path
    public function first($conditions = []) {
        $store = parent::first($conditions);
        if ($store) {
            $store = $this->normalizeLogoPath($store);
        }
        return $store;
    ...
    // Helper untuk memastikan logo path selalu dalam format yang benar
    private function normalizeLogoPath($store) {
        if (!empty($store['store_logo_path'])) {
            // Jika path tidak dimulai dengan '/', berarti hanya filename saja
            if (strpos($store['store_logo_path'], '/') !== 0) {
                $store['store_logo_path'] = '/uploads/logos/' . $store['store_logo_path'];
            }
        }
        return $store;
    }
    ...
    public function getStoreWithOwner($storeId) {
        ...
        return $store ? $this->normalizeLogoPath($store) : null;
    }
